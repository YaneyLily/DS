...
void List::insert(int item,int pos)//插入到第pos个节点之后的位置----------
{
    assert(pos>=0);//保证插入位置非负
    NodePointer ptr=first,
            newPtr=new node<int>(item);//定义一个带数据的节点
...
void List::import(const int *array,int size)//将数组元素导入链表的函数-----------------
{
    if(size>0)//保证数组元素个数大于0
    {
        NodePointer currPtr,newnode;//这两个指针将完成对数组数据的导入
        first=new node<int>(*array);//头指针指向第一个节点，同时将数组元素初始化在节点的数据域
        currPtr=first;
...
List List::merge(const List & aList)//归并链表函数------------------------
{
    ...
    mergeList.first=new node<int>(0);//归并链表的头指针指向一个数据域为0节点（此节点不是真正的头节点）
    ...
    while(ptr1!=0&&ptr2!=0)//ptr1与ptr2均不为0
    {
        ...
        {
            nextPtr=new node<int>(ptr1->data);//生成一个数据等于ptr1->data的节点
        ...
        {
            nextPtr=new node<int>(ptr2->data);//生成一个数据等于ptr2->data的节点
            currPtr->next=nextPtr;//currPtr指针的指针域存放新节点地址，完成前后节点的连接
            currPtr=nextPtr;//currPtr指针指向新节点
...
void List::changeFirst(NodePointer Pfirst) {
    first=Pfirst;
}

void List::getSize(int size) {
    mySize=size;
}

NodePointer List::getFirst() {
    return first;
}
